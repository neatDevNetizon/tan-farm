...
contract("TANVault", (accounts) => {
  ...
  describe("deposit", () => {
    it("allows to deposit token when contract is not paused", async () => {
      ...
      assert.equal(
        (await vault.calculateTotalPendingTANRewards()).toString(),
        "0"
      );
  ...
  describe("TANAtLastUserAction", () => {
    it("calculates the TAN correctly with balanceOf value after token safeTransfer", async () => {
      ...
      assert.equal(
        (await vault.userInfo(boredApe)).TANAtLastUserAction.toString(),
        "500",
        "from 500*500/500"
      );
      ...
      assert.equal(
        (await vault.userInfo(penguin)).TANAtLastUserAction.toString(),
        "1500",
        "from 1500*2000/2000"
      );
